...
# ── Branch path locals ────────────────────────────────────────
#
# Pre-computed at the root level so that ValidateConfig for
# conjur_policy_branch resources always sees a concrete string.
# (cyberark/conjur ValidateBranch does not guard against unknown
# values; child-module input variables are unknown during the
# early ValidateConfig pass.)
locals {
...
resource "conjur_authenticator" "jwt" {
  ...
  data = {
    ...
    identity = {
      identity_path      = local.branch_workloads
...
#
# Equivalent to:
#   conjur policy load -b data -f policy-tree.yaml
#   conjur policy load -b data -f workload-identities.yaml
